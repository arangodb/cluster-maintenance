...
/**
 * Pure analysis functions used in analyze.js.
 * These functions take an info object and dump object and populate info with
 * analysis results.
 */
  ...
  for (const [db, collections] of Object.entries(planCollections)) {
    ...
    for (const [name, col] of Object.entries(collections)) {
      ...
      for (const [shard, servers] of Object.entries(shards)) {
        ...
        const leader = servers[0].replace(/^_/, '');
        if (!_.has(info.primaries, leader)) {
            info.leaderOnDeadServer.push({db, name, shard, server: leader, servers});
        }
        for (const follower of servers.slice(1)) {
          if (!_.has(info.primaries, follower)) {
            info.followerOnDeadServer.push({db, name, shard, server: follower, servers});
          }
        }
      }
    }
...
const extractDistributionGroups = (info, dump) => {
  ...
  */
    ...
    for (const [cid, col] of Object.entries(collections)) {
      ...
      for (const [shard, servers] of Object.entries(shards)) {
        try {
          ...
          // TODO should this handle resigned/resigning leaders differently?
          ...
          if (servers.length > 1 && curServers.length <= 1) {
            ...
            // TODO should we strip the underscore of a resigned leader
  ...
  // TODO unplannedLeader is not reported nor used anywhere
...
const extractOutOfSyncFollowers = (info, dump) => {
  ...
  const compareFollowers = (plan, current) => {
    // TODO handle resigning/resigned leaders specially?
