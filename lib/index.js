...
(function () {
    ...
    arango: function (args, options) {
      ...
      try {
        if (arango && arango.isConnected()) {
          let v = arango.GET("/_admin/version");
          if (v.license != "enterprise" && !options.force) {
            fatal("these scripts are supposed to be used with the Enterprise Edition of ArangoDB");
          }
        }
      } catch (err) {
        // this is not an error that justifies aborting the startup
        print("Not connected to an arangod instance");  
      }
    },
  ...
  const options = helper.checkGlobalArgs(ARGUMENTS);
