  ...
  if (!_.isEmpty(mismatch)) {
    ...
    _.each(mismatch, function(shards, database) {
      _.each(shards, function(shard) {
        if (!toHash.hasOwnProperty(database)) {
          toHash[database] = {};
        } 
        if (!toHash[database].hasOwnProperty(shardToCol[shard.name].name)) {
          toHash[database][shardToCol[shard.name].cid] = [shard.name];
        } else {
          toHash[database][shardToCol[shard.name].cid].push(shard.name);
        }
        table.addRow(
...
function detectHashMismatch(coordinatorEndpoint, agLeader, conf, toHash) {
  const _ = require('lodash');
  const AsciiTable = require('../3rdParty/ascii-table');
  const helper = require('../helper.js');
  ...
  _.each(toHash, function(collections, database) {
      ...
      _.each(shards, function (shard) {
        ...
        _.each(planCollections[database][collection].shards[shard], function (server) { 
          ...
          jobs.push({ shortname: healthRecords[server].ShortName,
                      endpoint: healthRecords[server].Endpoint, jobId: arango.GET_RAW(
                        "/_db/" + database + "/_api/collection/" + shard + "/checksum",
        ...
        while (jobs.length > 0) {
          ...
          while (i--) {
            ...
            if (resp.code === 200) {
              checksums[jobs[i].shortname] = resp.checksum;
              jobs.splice(i, 1);
            }
          }
        }
        ...
        var i = 0;
        var off = false;
        _.each(checksums, function(checksum, shortname) {
          if (i > 0) {
            if (last != checksum) {
              off = true;
              return;
        ...
        if (off) {
          let cname = planCollections[database][collection].name;
          if (!mismatch.hasOwnProperty(database)) {
            mismatch[database] = {};
          }
          if (!mismatch[database].hasOwnProperty(collection)) {
            mismatch[database][cname] = {};
          }
          mismatch[database][cname][shard] = checksums;
        }
      });
    ...
    _.each(mismatch, function(collections, database) {
      _.each(collections, function(shards, collection) {
        _.each(shards, function(hashes, shard) {
          table.addRow(database, collection, shard);
        });
      });
    });
    print(table.toString());
...
exports.run = function (extra, args) {
  ...
  var trxId = openStreamingTrx(coordinatorEndpoint, "_system", "_users");
  closeStreamingTrx(coordinatorEndpoint, "_system", trxId);

  const toHash = detectCountMismatch(agLeader, conf);

  if (!_.isEmpty(toHash)) {
    detectHashMismatch(coordinatorEndpoint, agLeader, conf, toHash);
  }
