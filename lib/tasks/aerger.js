...
function createTwoCollectionsSameLeaderAndFollower(database, cn, cn2) {
  db._useDatabase("_system");
  try {
    db._createDatabase(database);
  } catch (e1) {
  ...
  db._create(cn2, {numberOfShards:1, replicationFactor:2});
  // Get dbserver names first:
  let health = arango.GET("/_admin/cluster/health").Health;
  let endpointMap = {};
  let mapShortToLong = {};
  ...
  let plan2 = shardDist[cn2].Plan;
  let shards = Object.keys(plan);
  let shards2 = Object.keys(plan2);
  let coordinator = "Coordinator0001";
  let leader = plan[shards[0]].leader;
  let follower = plan[shards[0]].followers[0];
  // Make leaders the same:
  if (leader !== plan2[shards2[0]].leader) {
    let moveShardJob = {
      database: database,
      collection: cn2,
      shard: shards2[0],
      fromServer: plan2[shards2[0]].leader,
      toServer: leader,
      isLeader: true
    };
    let res = arango.POST("/_admin/cluster/moveShard", moveShardJob);
    ...
    while (true) {
      ...
      plan = arango.GET("/_admin/cluster/shardDistribution").results[cn].Plan;
      if (plan[shards[0]].followers.length === 1) {
        break;
      }
      internal.wait(1);
    }
  ...
  // Make followers the same:
  if (follower !== plan2[shards2[0]].followers[0]) {
    let moveShardJob = {
      database: database,
      collection: cn2,
      shard: shards2[0],
      fromServer: plan2[shards2[0]].followers[0],
      toServer: follower,
      isLeader: false
    };
    let res = arango.POST("/_admin/cluster/moveShard", moveShardJob);
  ...
  return { endpointMap, coordinator, leader, follower, shards, shards2, cn, cn2, mapShortToLong, agencyLeader, database };
}
...
exports.run = function(extra, args) {
  ...
  collInfo = createTwoCollectionsSameLeaderAndFollower(database, "badcoll", "goodcoll");
