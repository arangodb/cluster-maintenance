...
exports.run = function (extra, args) {
  ...
  if (shardsToFix === undefined) {
    print("Need sharding strategy file as first argument. Giving up.");
    process.exit(1);
  }
  let runDetection = helper.getValue("run-detect", args);
  print("Hallo0");
  if (runDetection) {
    print("Hallo1", extra.tasks);
    let subArgs = helper.checkArgs(extra.tasks["detect-permanently-out-of-sync"], ["true", "false"]);
    print("Hallo2", subArgs);
                ...
                if (server != planLeader && check == checkValue) { // TODO FIX
                  ...
                  if (typeof(result) == "object" && result.hasOwnProperty("results") &&
                    ...
                    if (typeof(result) == "object" && result.hasOwnProperty("results") &&
                      ...
                      print ("waiting for current ");
                      while(require('internal').time() - start < 600.0) {
                        let currentServers = arango.POST("/_api/agency/read", [[curPath]])[0].
                            arango.Current.Collections[database][cid][shard].servers;
                        currentServers.sort();
                        if (_.isEqual(planServersSorted, currentServers)) {
