...
exports.run = function (extra, args) {
  ...
  const backup = arango.POST('/_admin/backup/create', {});
            ...
            if (planLeader == checkLeader) { // Leader must still be the same as for the checks
              _.each(servers, function (serverData) {
                ...
                if (server != planLeader && check != checkValue) {
                  ...
                  const maintenance = arango.PUT('/_admin/cluster/maintenance', '"on"');
                  print('... done');
                  ...
                  var result;
                  var result = arango.POST('/_api/agency/write', [removeFollowerTrx]);
                  if (typeof (result) === 'object' && result.hasOwnProperty('results') &&
                    ...
                    if (typeof (result) === 'object' && result.hasOwnProperty('results') &&
                      ...
                      const maintenance = arango.PUT('/_admin/cluster/maintenance', '"off"');
                      print('... done');
                      arango.reconnect(agLeader, '_system');
